"""
Modified OpenAI Gym blackjack env with card counting
...
class BlackjackWithCountEnv(gym.Env):
    """
    ...
    * v0: Initial versions release (1.0.0)
    ...
    def __init__(self, render_mode: Optional[str] = None, natural=False, sab=False):
        ...
        self.observation_space = spaces.Tuple(
            (spaces.Discrete(32), spaces.Discrete(11), spaces.Discrete(3), spaces.Discrete(41))
        )
    ...
    def set_rnd_seed(self, seed=None):
    ...
    def step(self, action):
        ...
        if action == 1:  # hit: add a card to players hand and return
            ...
            self.plus_minus_sum += self._get_card_cost(card)
            ...
            if self.sab and is_natural(self.player) and not is_natural(self.dealer):
                ...
                reward = 1.5
        elif action == 2:  # double: double and finish the game
            terminated = True
            card = self._draw_card(self.np_random)
            self.plus_minus_sum += self._get_card_cost(card)
            self.player.append(card)
    ...
    def _get_obs(self):
        return (sum_hand(self.player), self.dealer[0], usable_ace(self.player), self._get_strategy_value())
    ...
    def reset(
        ...
        self.plus_minus_sum += self._get_card_cost(dealer_hand[0])
        self.dealer = dealer_hand
        player_hand = self._draw_hand(self.np_random)
        self.plus_minus_sum += self._get_card_cost(player_hand[0])
        self.plus_minus_sum += self._get_card_cost(player_hand[1])
    ...
    def _get_strategy_value(self):
        return self.plus_minus_sum


    def _get_card_cost(self, card):
        return PLUS_MINUS_COST_DICT[card]
